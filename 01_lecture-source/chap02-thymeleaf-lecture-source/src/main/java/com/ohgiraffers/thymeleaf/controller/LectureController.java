...
@Controller
@RequestMapping("/lecture")     //lecture 중복으로 @RequestMapping 중복된것 선언해준다
public class LectureController {
    ...
    public ModelAndView expression(ModelAndView mv) {
        ...
        *  생각을 해보자
        *  */
        mv.addObject("member", new MemberDTO("이도아", 20, '여', "경기도 김포시"));

        mv.addObject("hello", "hello <h3>thymeleaf!!</h3>");

        mv.setViewName("/lecture/expression");  // 보여질 것을 내가 정한다  (@GetMapping("/lecture/expression") index 에서 가져옴 => 정적인 페이지 에서 여기로 가져와서 동적이게 만듬)

        return mv;  // void 형이 아니기 때문에 return 필요
    }
    ...
    public ModelAndView conditional(ModelAndView mv) {
        ...
        memberList.add(new MemberDTO("이권한", 30, '여', "서울시 영구"));

        mv.addObject("memberList", memberList);

        return mv;
        // 요청 주소 받는 주소가 같으면 view 이름 생략가능
//        mv.setViewName("/lecture/conditional");
    ...
    public ModelAndView etc(ModelAndView mv) {
        ...
        mv.addObject(selectCriteria);    // selectCriteria 값 자체가 key 이면서 value 이다

        MemberDTO member = new MemberDTO("이도아", 20, '여', "서울시 노진구");
